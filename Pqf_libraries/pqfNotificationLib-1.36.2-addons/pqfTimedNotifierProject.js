...
const OUTBOX_PROCESSING_SCHEDULE = {
	...
	"weekly": [1, 2, 3, 4, 5, 6, 7] // Optional. Array with days of week, where Sunday = 0, Monday = 1, e.g., [2, 4] means every Tuesday and Thursday, [1,2,3,4,5] means every working day	
};
...
[
	...
	// An exemplary rule for criterion-based triggering PROJECT notifications WITH REMINDERS
	// ------------------------------------------------------
    {
        "objectType": "Project",
		"schedule": {
			"weekly": [1, 2, 3, 4, 5]
			// On these days notifications can be generated. A notification
			// for a specific project will be generated once and only once.
			// But reminders for such notifications can be sent (via outbox).
		},
		"projectFilter": { // Consider only projects that match this filter
			...
			"indicatorTargets": [ // Compares actuals or forecasts with targets
				{
					"IndicatorDetailDefinitionId": "PM-IND-DIMENSION-TERMINE-MS50",
					"actual": [ // "actual" | "forecast"
						{
							"comparison": "absolute", // "absolute" | "relative"
							"logic": "or", // "or" | "and"
							"largerThan": 5, // --> actual > target + 5
							//"smallerThan": 10, // --> actual < target - 10
						}
					]
				}
			]
		},
		// Decide a reminder goes to the outbox
		// (if "outboxOptions" exists) and how often and when
		// it will be sent from there
		"outboxOptions": {
			//"repeatTimes": 3, // Send this many time, each time when the outbox is processed
			//"notBeforeDays": 1, // Goes together with "repeatTimes"
			//"sendAtDays": [5, 8] // Alternative to "repeatTimes": Send reminder this many days after the notification was triggered
			"schedule": { // Send a reminder regularly and *indefinitely* (until an event removes it from the outbox)
				"monthly": REMINDER_DAYS // On these days notifications can be generated. But a notification for a specific project won't be triggered again. Only reminders can be sent (via outbox).
			},
		},
		// Decide who will be notified (and reminded)
		...
		// Decide how the email looks like
		"useEmailTemplate": createProjectClosureReminder,
		// Decide who gets a summary of what happened
		"contacts": CONTACT_EMAILS.map(function(email) {
			return {
				"name": null,
				"email": email,
				"useEmailTemplate": createProjectClosureReminderSummary
			};
		})
    },
	
	// An exemplary rule for criterion-based removing PROJECT reminders from the outbox
	// ----------------------------------------------------------------
    {
        "objectType": "Project",
		"schedule": {
			"weekly": [1, 2, 3, 4, 5, 6, 7]
			// On these days reminders can be removed
		},
		"projectFilter": { // Consider only projects that match this filter
			"lifecycleStates": ACTIVE_PROJECT_STATE_IDS,
			"properties": [
			...
			"indicatorTargets": [ // Compares actuals or forecasts with targets
				// Rule that compares today's date (time indicator)
				// or the current value (costs or effort indicator)
				// of an indicator with its target value
				// If more than one check are defined (array),
				// all of them must be met.
				//
				// *** Currently only implemented for time indicators ***
				{
					"reportType": "ProjectReport",
					"indicatorDetailDefinitionId": "PM-IND-DIMENSION-TERMINE-MS50",
					"checks": [
						{
							"comparison": "absolute", // "absolute" (currently only this parameter is implemented)
							"equalTo": 5, // --> today = target + 5 days
							//"smallerThan": 10, // --> today < target - 10 days
							//"smallerThanOrEqualTo": 10, // --> today <= target - 10 days
							//"greaterThan": 10, // --> today > target - 10 days
							//"greaterThanOrEqualTo": 10, // --> today >= target - 10 days
						}
					]
				}
			]
		...
		"notifyStaticEmails": NOTIFY_EMAIL_ADRESSES, // Will be notified about the removal of an item from the outbox
		"notifyOptions": {
			"forceNotify": NOTIFY_UNCONDITIONALLY,
			"defaultLanguage": "de"
		},
		"useEmailTemplate": createProjectReminderRemoval
    }
...
function createProjectClosureReminder(receiverInfos, objectInfos, period, language, contact) {
	let subject = "";	
	switch(language) {
		case "en":
...
function createProjectClosureReminderSummary(contact, period, statistics) {
